...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	u8 BUF [2];
	BUF [0] = reg + 2;//command byte
	BUF [1] = val;//data code

	//write val via SPI
	int rval = XSpi_Transfer(&SpiInstance, BUF, NULL, 2);
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 BUFF[2];
	BUFF[0] = reg + 2;//command byte
	int n;

	//loop to write each data byte to the register (FIFO)
	for(n=0; n<nbytes; n++){
		BUFF[1]=data[n];//data byte
		int rval = XSpi_Transfer(&SpiInstance, BUFF, NULL, 2);
		if(rval != 0){	//read return code from SPI peripheral
			//if return code != 0 print an error
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//select MAX3421E (
	XSpi_SetSlaveSelect(&SpiInstance,SpiInstance.SlaveSelectMask);
	//make vars for read and write
	u8 reg0 [2];
	reg0[0]=reg;
	reg0[1]=0;//initialize data byte


	u8 val[2] = {0, 0}; // Initialize both elements to zero

	//write reg via SPI
	//read val via SPI
	int rval = XSpi_Transfer(&SpiInstance, reg0, val, 2);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for (n=0; n<nbytes-1; n++){
		int rval = XSpi_Transfer(&SpiInstance, reg0, val, 2);
		data[n]=val[1];//read data from SPI
		//read return code from SPI peripheral
		//if return code != 0 print an error
		if(rval!=0){
			xil_printf("MAX READ ERROR");
		}
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
